...
// Load CSV Functionality (1 B+ Tree per Ticker)
std::map<std::string, BPlusTree> CSVLoader::loadCSV_BPlusTree(const std::string& filepath, int order) {
    std::map<std::string, BPlusTree> trees;
    std::ifstream file(filepath);
    ...
    while (std::getline(file, line)) {
        ...
        auto [it, inserted] = trees.try_emplace(ticker, order);
